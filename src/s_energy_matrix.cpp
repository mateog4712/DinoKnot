...
s_energy_matrix::s_energy_matrix (std::string seq, cand_pos_t length, short *S, short *S1, vrna_param_t *params1, vrna_param_t *params2)
// The constructor
{
    params_ = params1;
	params2_ = params2;
...
*/
energy_t s_energy_matrix::E_MLStem(const energy_t& vij,const energy_t& vi1j,const energy_t& vij1,const energy_t& vi1j1,const short* S, paramT* params,paramT *params2,cand_pos_t i, cand_pos_t j, const  cand_pos_t& n, std::vector<Node> &tree){
...
void s_energy_matrix::compute_energy_WM_restricted_emodel (cand_pos_t i, cand_pos_t j, sparse_tree &tree, std::vector<energy_t> &WMB)
// compute de MFE of a partial multi-loop closed at (i,j), the restricted case
{
	...
	if (tree.tree[j].pair <= -1) m5 = std::min(m5,WM[ijminus1] + emodel_energy_function(i,j,params_->MLbase,params2_->MLbase));
...
void s_energy_matrix::compute_VMprime(cand_pos_t i, cand_pos_t j, sparse_tree &tree, std::vector<energy_t> &WMB){
	...
	for (cand_pos_t k = i+4; k <= j-3; ++k){
		cand_pos_t kj = index[k]+j-k;
		m1 = std::min(m1,get_energy_WM(i,k-1) + E_MLStem(get_energy(i,j),get_energy(i+1,j),get_energy(i,j-1),get_energy(i+1,j-1),S_,params_,params2_,i,j,n,tree.tree));
